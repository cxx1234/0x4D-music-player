...
> 结果：`feature/audioplayers-engine`（5 个提交，tip `05769b0`）以 `--no-ff` 合入本地 `dev`；
> `dev` 领先 `origin/dev` **6 个提交、未推送**（详见 §3.1 / §9）
...
| # | 提交 | 内容 |
|---|---|---|
| 1 | `aa64116 docs(audio): add playback engine migration plan` | 本文件 |
| 2 | `0e32329 refactor(audio): introduce single-track AudioEngine abstraction` | 接口 + `AudioplayersEngine` + `PlayerService` 重写 |
| 3 | `b6913d3 fix(player-ui): harden queue scroll restore and error toasts` | 附录 D 的滚动加固 + 错误提示替换 |
| 4 | `9c99310 test(player): cover queue semantics and bad-file handling` | 假引擎单测 + `tool/make_bad_audio.py` |
| 5 | `05769b0 refactor(audio): drop just_audio dependency` | 移除旧依赖 + 文档同步 |

两处与计划不同：

* **未产出"单曲化的 `JustAudioEngine`"中间产物**（Phase 1 的隔离验证改由 Phase 2 的假引擎单测承担）；
* **未跑完整的"两个构建产物 A/B"流程**：Phase 4 改为**真机手测 + 坏文件素材实测**（附录 E），
  `dev` 只在对照时构建过一次。

合入方式：`git merge --no-ff`，合并信息沿用项目既有格式（首行 `Merge branch '...' into dev` +
摘要段落 + 要点列表 + `Verified:` 行）；合并提交的父提交为 `368b05d`（原 `dev`）与 `05769b0`（分支 tip），
22 文件（净增约 2100 行、删除 547 行）。合并后在 `dev` 上复验：`flutter analyze` 0 告警、`flutter test` **209 通过**。

> 本文件随该合并提交一并落库，因此不引用合并提交自身的哈希（amend 会让它变动）：
> 查看方式 `git log --merges -1 dev`。

`feature/audioplayers-engine` 分支仍保留在本地（提交已可从合并提交到达，可安全 `git branch -d`）。
...
**实际结果（2026-09-17）**：**采纳**（未走 PR，直接在本地以 `--no-ff` 合入 `dev`）。
`Architecture.md` / `README.md` / `CHANGELOG.md` / `docs/TODO.md` 与 `/memories/repo/` 均已同步；
`dev` **尚未推送**。
...
**采纳（2026-09-17）**：引擎已切换为 `audioplayers`，`JustAudioEngine` 与 `just_audio` /
`just_audio_platform_interface` 依赖已删除；`AudioEngine` 抽象落地为**单曲**接口，
队列语义（顺序推进 / 随机排列 / 重复模式 / 队尾收尾 / 自动跳过）全部由 `PlayerService` 持有。

| 项 | 结果 |
|---|---|
| 曲间表现 | 使用者曲库中没有连续型专辑，实测无可感知退化（见 §0.4 / §2.1） |
| 尾部截断 | 实测未发现（Phase 4 的硬性检查项） |
| 切歌延迟 | 无可感知差异 |
| 坏文件处理 | 实测暴露 4 个问题，两轮修复后全部通过（附录 E） |
| 滚动视图 | 顺带修复一处既有竞态（附录 D） |
| 构建 | macOS 纯 SPM，无 Podfile；`just_audio` / `audio_session` 已从插件注册中移除 |
| 测试 | 全量 209 通过（含 11 例假引擎队列语义测试） |

**收益**：不再存在"引擎镜像队列"，队列编辑（加歌 / 插队 / 删歌 / 拖动排序）对引擎零操作；
队列逻辑可单测（此前完全依赖真机）。
**代价**：失去 gapless / 预取（使用者的曲库对此不敏感）；每次换源多一次显式 `pause()` 对齐状态。

**合并（2026-09-17）**：`feature/audioplayers-engine` 以 `--no-ff` 合入本地 `dev`（合并提交见 §3.1），
合并后在 `dev` 上复验 `flutter analyze` 0 告警、`flutter test` **209 通过**、macOS debug 构建成功（纯 SPM）。
`dev` 领先 `origin/dev` **6 个提交**，**未推送**；分支提交见 §3.1。

**后续（不属于本次迁移）**：Windows 适配（`SandboxService` 平台化 / CMake 3.15 / `duration` 空值兼容）、
双实例预加载（附录 C、§8-Q6，已暂缓）、`PlayerService` 拆分（§8-Q4）。
